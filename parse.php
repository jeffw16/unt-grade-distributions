...
for ( $i = 1; $i < count( $arr ); $i++ ) 
{
	...
	if ( ($i > 0 && sizeof($narr) > 0 && $arr[$i - 1][2] == $linearr['subj'] && $arr[$i - 1][3] == $linearr['num'] && $arr[$i - 1][4] == $linearr['sect'])) // if section already exists in array
	{			
		$narr[sizeof($narr) - 1]['grades'][utf8_encode($arr[$i][9])] = utf8_encode($arr[$i][10]);
	}	  
	else // if section does not already exist in the array
	{
...
for ( $i = 1; $i < count($narr); $i++) // eliminates entries in which the only grades are ungraded, or ''
{		
	if (count($narr[$i]['grades']) == 1 && $narr[$i]['grades'][''] > 0) 
	{
		unset($narr[$i]); // remove the element
	}
}
